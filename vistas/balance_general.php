<div class="container mt-5">
  <h1 class="title is-3">Captura de Balance General</h1>
  ...
  <?php
    ...
    // Obtener cuentas de balance del catálogo
    $cuentas_balance = [];
    if (!empty($empresa_id)) {
        $cuentas_balance = $conn->query("SELECT id, nombre, tipo FROM catalogo_cuentas WHERE reporte = 'balance'")->fetchAll();
    }
    
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['guardar_balance'])) {
      $empresa_id = $_POST['empresa_id'];
      ...
      try {
          ...
          $stmt_delete = $conn->prepare("DELETE FROM cuentas_balance WHERE periodo_id IN 
                        (SELECT id FROM periodos WHERE empresa_id = ?)");
          $stmt_delete->execute([$empresa_id]);
          
          // Preparar la inserción de nuevas cuentas
          $stmt_insert = $conn->prepare("INSERT INTO cuentas_balance 
          ...
          foreach ($cuentas as $index => $cuenta_nombre) {
              if (!empty($cuenta_nombre)) {
                  ...
                  $stmt_find = $conn->prepare("SELECT id FROM catalogo_cuentas 
                                          WHERE nombre = ? AND reporte = 'balance' LIMIT 1");
                  $stmt_find->execute([$cuenta_nombre]);
                  $cuenta_id = $stmt_find->fetchColumn();
                  
                  // 2. Si no existe, crearla y asignarle un nuevo orden
                  if (!$cuenta_id) {
                      // Obtener el máximo orden actual para el reporte de balance
                      $max_orden = $conn->query("SELECT MAX(orden) FROM catalogo_cuentas WHERE reporte = 'balance'")->fetchColumn();
          ...
          $conn->commit();
          echo "<div class='notification is-success'>Balance General guardado exitosamente.</div>";
  ...
  <form method="post" class="form-wide">
    ...
    <?php if (!empty($periodos)): ?>
      <h2 class="subtitle is-5 mt-4">Cuentas del Balance General</h2>
      ...
      <div class="table-container" style="overflow-x: auto;">
        <table class="table is-bordered is-fullwidth" id="tablaCuentas">
          ...
          <tbody>
            <tr>
              <td><input class="input" type="text" name="cuenta[]" placeholder="Caja, Bancos, etc." required></td>
              <td>
      ...
      <div class="field mt-4">
        <button type="submit" name="guardar_balance" class="button is-primary">Guardar Balance General</button>
      </div>
    <?php endif; ?>
  </form>
</div>
...
<script>
  function agregarFila() {
    ...
    nuevaFila.querySelector('select[name="tipo[]"]').value = "activo";
